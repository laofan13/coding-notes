...
public:
	...
	enum class State { INIT, RUNNING, STOP };
	...
	void AddTaskWithTimePoint(const std::chrono::time_point<std::chrono::high_resolution_clock> &time_point, F &&f,
	                          Args &&...args) {
		TimerTask t;
	...
	void CancelRepeatedTask(int id) {
		repeated_id_state_map_.erase(id);
	}
...
int main() {
	...
	timer.Run();
	for (int i = 5; i < 15; ++i) {
		timer.AddTaskAfterDuration(std::chrono::seconds(i + 1), [i]() { std::cout << "this is " << i << std::endl; });

		timer.AddTaskWithTimePoint(
		    std::chrono::high_resolution_clock::now() + std::chrono::seconds(1),
		    [](int i) { std::cout << "this is " << i << " at " << std::endl; }, i);
	}

	int id = timer.AddRepeatedTask(10, std::chrono::seconds(1), []() { std::cout << "repeate task " << std::endl; });
