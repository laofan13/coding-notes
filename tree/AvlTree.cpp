...
static Node *leftRotate(Node *root) {
	...
	right->height = std::max(right->left ? right->left->height : 0, right->left ? right->left->height : 0) + 1;
...
static Node *rightRotate(Node *root) {
	...
	root->height = std::max(root->left ? root->left->height : 0, root->right ? root->right->height : 0) + 1;
	left->height = std::max(left->left ? left->left->height : 0, left->left ? left->left->height : 0) + 1;
